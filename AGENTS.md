...
**Responsibilities**:
- Clean React with hooks, Vite build system components (`.jsx`), one per file, in `src/components/` or `src/views/`
- Every UI feature must work with **mock data before live data is wired up** — the mock shape must match the real API response
- Amber (#F0A830) for primary actions and highlights, teal (#2DD4A8) for positive metrics, coral (#F87171) for drop-offs and issues
...
**Outputs**: React with hooks, Vite build system components (`.jsx`) in `src/components/`, `src/views/`
...
- Customer.io: batch API calls — avoid one HTTP request per record
- Match Customer.io profiles to other records by email address (most reliable join key)
...
- Test Docker builds locally before deploying: `docker build -t solar-journey-xdemandiq . && docker run -p 8080:8080 solar-journey-xdemandiq`
...
claude --add-dir .
# Paste: "You are the Frontend Agent for solar-journey-platform. [Track B brief here]"

# Terminal 2 — start data-integration agent
claude --add-dir .
# Paste: "You are the Data Integration Agent for solar-journey-platform. [Track A brief here]"
```
